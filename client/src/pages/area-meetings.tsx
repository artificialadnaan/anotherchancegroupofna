...
import { MapPin, Clock, Search, Phone, Globe, Accessibility } from "lucide-react";
...
const areaGroups: AreaGroup[] = [
  ...
  },
  {
    ...
    schedule: {
      Sunday: [slot("6:00 PM")],
      Monday: [slot("6:00 PM")],
      Tuesday: [slot("6:00 PM")],
      Wednesday: [slot("6:00 PM")],
...
function DayColumn({ day, slots }: { day: Day; slots: MeetingSlot[] }) {
  if (slots.length === 0) {
    return (
          ...
          <div>
            ...
            <div className="flex items-start gap-1.5 mt-1 text-sm text-muted-foreground">
              ...
              <a
                ...
                data-testid={`link-map-${group.name.toLowerCase().replace(/\s+/g, "-")}`}
              >
                {group.address}
              </a>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-sm border-t">
            ...
            <tbody>
              <tr>
                {days.map((day) => (
                  <DayColumn key={day} day={day} slots={group.schedule[day]} />
                ))}
              </tr>
            </tbody>
          </table>
...
export default function AreaMeetingsPage() {
  ...
  const totalMeetings = filteredGroups.reduce(
    (sum, g) =>
      sum +
      days.reduce((ds, day) => ds + g.schedule[day].length, 0),
    0
  );
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      <div className="mb-6 md:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold mb-2">Fort Worth Area Meetings</h1>
        <p className="text-muted-foreground mb-4">
          Browse all Narcotics Anonymous meetings in the Fort Worth area. Click any address to open it in Google Maps.
        </p>
      ...
      <div className="mb-6 p-3 sm:p-4 bg-muted/50 rounded-md">
